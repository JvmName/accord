...
class SoloMatchSessionScoreTest {
    ...
    fun `3 RED holding ticks yields 1 red point`() = runTest {
        ...
        repeat(3) { tracker.emit(ButtonEvent.Holding(Competitor.RED)) }
        advanceUntilIdle()
    ...
    fun `2 RED holding ticks yields 0 red points (below threshold)`() = runTest {
        ...
        repeat(2) { tracker.emit(ButtonEvent.Holding(Competitor.RED)) }
        advanceUntilIdle()
    ...
    fun `9 RED holding ticks yields 3 red points`() = runTest {
        ...
        repeat(9) { tracker.emit(ButtonEvent.Holding(Competitor.RED)) }
        advanceUntilIdle()
    ...
    fun `red holds 3 ticks then blue holds 6 ticks yields 1 red point and 2 blue points`() = runTest {
        ...
        repeat(3) { tracker.emit(ButtonEvent.Holding(Competitor.RED)) }
        advanceUntilIdle()
        tracker.emit(ButtonEvent.Release(Competitor.RED))
        advanceUntilIdle()

        // Blue holds 6 ticks (2 points), accumulating from a fresh session
        repeat(6) { tracker.emit(ButtonEvent.Holding(Competitor.BLUE)) }
        advanceUntilIdle()
    ...
    fun `72 RED holding ticks triggers tech fall win for RED (round 1 maxPoints=24)`() = runTest {
        ...
        assertEquals(Competitor.RED, session.score.value.techFallWin)
    }
    ...
    fun `72 BLUE holding ticks triggers tech fall win for BLUE (round 1 maxPoints=24)`() = runTest {
        ...
        repeat(72) { tracker.emit(ButtonEvent.Holding(Competitor.BLUE)) }
        advanceUntilIdle()

        assertEquals(Competitor.BLUE, session.score.value.techFallWin)
    }
}
